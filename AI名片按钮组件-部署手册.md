...
2. **`__POSTER_URL__` 指的图必须真实存在**——复刻新卡时最常踩:网页照抄了,海报却没生成/没上传到那个地址,点按钮弹层就是一张裂图且毫无提示。本组件已内置 `onerror` 兜底(显示"海报还没生成"+回显缺失URL);另注意 `.poster-box img` 的 `display:block` 会盖掉 `hidden` 属性,必须配 `img[hidden]{display:none}` 才藏得住裂图。
3. **海报图改版必须换 `?v=N` 版本号**——微信/浏览器缓存旧图,「明明部署了还是旧版」九成是这个
4. **微信内不能用 `<a download>` 下载、不能直接拉起转发**——所以保存走"长按图片"、分享走"右上角引导层",别试图绕过
5. **iOS 长按保存依赖 `-webkit-touch-callout:default`**——别在海报 img 上全局禁用 callout
6. **隐藏图片别用 `display:none`**(本组件不涉及,但同页放微信分享缩略图时:用 `left:-9999px`,display:none 会被微信爬虫跳过)
7. **`execCommand('copy')` 失败不抛异常**——必须检查返回值,否则会弹假成功 toast
